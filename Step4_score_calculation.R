# This script was used to calculate the ranking score for each Kinetics previously collected based on different criteria
...
D$Dvalues_tab <- dplyr::arrange(D$Dvalues_tab, Kinetics_key)
metadata <- dplyr::arrange(metadata, Kinetics_key)

DATASET <- cbind(metadata, 
                 dplyr::select(D$Dvalues_tab, -c("Kinetics_key")))
...
for (i in 1:nrow(DATASET)) { # for each Kinetics: assign a score for different categories 
...
openxlsx::write.xlsx(DATASET, file = "DataRecords_OutputData.xlsx")
