...
df <- data.frame(Kinetics_key = character(),
                 time_hours = numeric(),
...
for (i in 1:length(dd$scatterplot)) { # for each digitalized figure
  ...
  df <- rbind(df, data.frame(Kinetics_key = dd$scatterplot[[i]]$id, # 'id' (Kinetics_key),
...
dk$Kinetics_key <- as.character(dk$Kinetics_key)
dk <- arrange(dk, Kinetics_key)
dk$Kinetics_key <- as.factor(dk$Kinetics_key)
