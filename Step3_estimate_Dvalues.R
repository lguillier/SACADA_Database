...
f_estimate_Dvalues = function(dk) {
  ...
  for (i in 1:nb_kinetics) { # for each kinetics
    ...
    regplot[[Kinetic_i]] <- ggplot(data = tab_Kinetic_i,
      ...
      coord_cartesian(xlim = c(min(tab_Kinetic_i$time_hours[!is.na(tab_Kinetic_i$y)]),
                               max(tab_Kinetic_i$time_hours[!is.na(tab_Kinetic_i$y)]))) +
      xlab("time (hours)") + 
      ylab(unique(dk$y_variable[dk$Kinetics_key == Kinetic_i]))
